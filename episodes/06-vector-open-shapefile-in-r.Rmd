...
R. Call the MaineDOT_Public_Roads object `lines_maine` and the Maine_Boat_Launches_GeoLibrary
`points_maine`.
...
lines_maine <- st_read("data/maine_gov_maps/MaineDOT_Public_Roads/MaineDOT_Public_Roads.shp")
points_maine <- st_read("data/maine_gov_maps/Maine_Boat_Launches_GeoLibrary/Maine_Boat_Launches_GeoLibrary.shp")
```
...
st_bbox(lines_maine)
st_crs(points_maine)
st_bbox(points_maine)
```

To see the number of objects in each file, we can look at the output from when
we read these objects into R. `lines_maine` contains 100669 features (all lines) and
`points_maine` contains 578 points.
